...
public class PlayerListener implements Listener {
	...
	public PlayerListener(WorldGuardPlugin plugin, RegionContainer container, SessionManager session) {
		this.plugin = plugin;
	...
	public void onPlayerDeathEvent(PlayerDeathEvent event) {
		...
		ApplicableRegionSet regions = container.createQuery().getApplicableRegions(localPlayer.getLocation());
	...
	public void onPlayerRespawnEvent(PlayerRespawnEvent event) {
		...
		Location respawnLocation = container.createQuery().queryValue(localPlayer.getLocation(), localPlayer, Flags.RESPAWN_LOCATION);
	...
	public void onPlayerItemConsumeEvent(PlayerItemConsumeEvent event) {
		...
		if (itemMeta instanceof PotionMeta potionMeta) {
			...
			session.get(plugin.wrapPlayer(player)).getHandler(GiveEffectsFlagHandler.class).drinkPotion(player, effects);
		} else {
			Material material = event.getItem().getType();

			if (material == Material.MILK_BUCKET) {
				session.get(plugin.wrapPlayer(player)).getHandler(GiveEffectsFlagHandler.class).drinkMilk(player);
			}
		}
	}
	...
	public void onPlayerGameModeChangeEvent(PlayerGameModeChangeEvent event) {
		Player player = event.getPlayer();
		Session wgSession = session.getIfPresent(plugin.wrapPlayer(player));
		...
		if (wgSession != null) {
			...
			if (value != null) {
				...
				}.runTask(WGExtraFlags.getPlugin());
			}
		} else {
			new BukkitRunnable() {
				@Override
	...
	private void checkFlyStatus(Player player, Boolean originalValueOverwrite) {
		FlyFlagHandler flyFlagHandler = session.get(plugin.wrapPlayer(player)).getHandler(FlyFlagHandler.class);
	...
	public void onPlayerItemDamageEvent(PlayerItemDamageEvent event) {
		...
		if (container.createQuery().queryState(localPlayer.getLocation(), localPlayer, Flags.ITEM_DURABILITY) == State.DENY) {
	...
	public void onPlayerSpawnLocationEvent(PlayerSpawnLocationEvent event) {
		...
		Location location = container.createQuery().queryValue(BukkitAdapter.adapt(event.getSpawnLocation()), localPlayer, Flags.JOIN_LOCATION);
	...
	public void onPlayerJoinEvent(PlayerJoinEvent event) {
		...
		Boolean value = session.get(plugin.wrapPlayer(player)).getHandler(FlyFlagHandler.class).getCurrentValue();
